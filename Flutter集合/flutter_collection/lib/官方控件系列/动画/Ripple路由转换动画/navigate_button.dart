...
class _NavigateButtonState extends State<NavigateButton> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _animationController = AnimationController(vsync: this, duration: Duration(milliseconds: time));
    ...
    return FloatingActionButton(
      ...
      onPressed: () {
        ...
        Future.delayed(Duration(milliseconds: time - 300))
            .then((_) => Navigator.of(context).push(FadeRouteBuilder(screen: widget.nextScreen)));
      },
      child: Icon(
        widget.icon,
    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        return Positioned(
          ...
          bottom: bottom - widget.rangeFactor * screenHeight * _animation.value,
          child: Container(
            alignment: Alignment.center,
            decoration: BoxDecoration(
              shape: BoxShape.circle,
              color:
                  _animation.value > 0.65 ? widget.splashColor.withOpacity(1 - _animation.value) : widget.splashColor,
            ),
            child: Icon(
              widget.icon,
              color: widget.iconColor,
  ...
  Future showOverlay(BuildContext context) async {
    ...
    OverlayEntry overlayEntry = OverlayEntry(builder: (context) => _ripple(context));
