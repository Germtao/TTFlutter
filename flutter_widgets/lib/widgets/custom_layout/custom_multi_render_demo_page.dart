...
class CustomMultiRenderDemoPage extends StatefulWidget {
  @override
  _CustomMultiRenderDemoPageState createState() => _CustomMultiRenderDemoPageState();
}
...
class _CustomMultiRenderDemoPageState extends State<CustomMultiRenderDemoPage> {
  ...
  Widget build(BuildContext context) {
    double childSize = 66.0;
    return Scaffold(
      appBar: AppBar(
        title: Text('CustomMultiRenderDemoPage'),
      ),
      body: Center(
        child: Container(
          ...
          child: CustomMultiChildLayout(
            ...
            children: [
              ...
              for (var item in customLayoutId)
                LayoutId(
                  id: item,
                  child: ContentItem(item, childSize),
                ),
            ],
          ),
        ),
...
class CircleLayoutDelegate extends MultiChildLayoutDelegate {
  ...
  CircleLayoutDelegate(this.customLayoutId, {this.center = Offset.zero, this.childSize});
  ...
  void performLayout(Size size) {
    for (var item in customLayoutId) {
      if (hasChild(item)) {
        ...
        childSize ??= Size(size.width / customLayoutId.length, size.height / customLayoutId.length);
