...
class HtmlUtils {
  static generateCode2Html(
    String mdData, {
    String backgroundColor = TTColors.miWhiteString,
    String lang = 'java',
    userBR = true,
  }) {
  ...
  static generateHtml(
    String mdData, {
    String backgroundColor = TTColors.webDraculaBackgroundColorString,
    userBR = true,
  }) {
    if (mdData == null) {
      return '';
    ...
    try {
      ...
      print('generate html error: ${e.toString()}');
    }
    ...
    return generateCodeHtml(
      mdDataCode,
      false,
      backgroundColor: backgroundColor,
      actionColor: TTColors.actionBlueString,
      userBR: userBR,
    );
  }
  ...
  static generateCodeHtml(mdHTML, wrap,
      {backgroundColor = TTColors.white, String actionColor = TTColors.actionBlueString, userBR = true}) {
    return "<html>\n" +
        "<head>\n" +
        "<meta charset=\"utf-8\" />\n" +
        "<title></title>\n" +
        ...
        "tr:nth-child(even) {" +
        "background:" +
        TTColors.primaryLightValueString +
        ";" +
        "color:" +
        TTColors.miWhiteString +
        ";" +
  ...
  static parseDiffSource(String diffSource, bool wrap) {
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      lineNumberStr = getDiffLineNumber(curRemoveNumber == -1 ? "" : (curRemoveNumber.toString() + ""),
  ...
  static resolveHtmlFile(var res, String defaultLang) {
    if (res != null && res.result) {
      ...
      if ('markdown' == lang) {
        return generateHtml(res.data, backgroundColor: TTColors.miWhiteString);
      } else {
        return generateCode2Html(res.data, backgroundColor: TTColors.webDraculaBackgroundColorString, lang: lang);
      }
    } else {
